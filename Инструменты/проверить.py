...
d = расчёт(EXC_ВНЕШ={'Form001': True})
проверка('блок 10 выключен · списание камер ноль, а не минус в «Амортизации съёмочного оборудования»',
         d['camAmort'] == 0 and d['camWear'] == 0 and d['depShoot'] - d['camAmort'] == 0,
         "camAmort {:,.2f} ₽/год · depShoot {:,.2f} ₽/год".format(d['camAmort'], d['depShoot']))
проверка('блок 10 выключен · обнуляются суммы обеих форм, а не строки',
         d['depShoot'] == 0 and abs(d['amort'] - (d['depOffice'] + d['depSoft'] + d['depWs'])) < 0.01,
         "амортизация прайса {:,.2f} ₽/год · суммы остальных блоков {:,.2f} ₽/год".format(
             d['amort'], d['depOffice'] + d['depSoft'] + d['depWs']))
