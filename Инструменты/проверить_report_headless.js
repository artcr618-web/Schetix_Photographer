...
async function render(name,override){
  ...
  assert(fail,ids.length===24&&new Set(ids).size===24,'не 24 уникальных блока');
  assert(fail,d.querySelector('.wp')?.children.length===24,'не 24 прямых блока .wp');
...
(async()=>{
  ...
  console.log(`Headless report: ${scenarios.length} сценариев · 24 блока · 12 таблиц · ошибок 0`);
