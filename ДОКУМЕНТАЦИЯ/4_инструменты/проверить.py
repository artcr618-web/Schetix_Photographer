...
def лояльность(d):
    ...
    for p_, s_, w in ((0.05,2,0.40),(0.07,4,0.40),(0.10,12,0.20)):
        ценаКл = s_*чек*p_
        n = max(round(min(бюдж*w/ценаКл if ценаКл else 0, цел*w/s_)), 1)
        пост += round(n*s_*чек*p_); заказы += n*s_
    ...
    ост = max(бюджСпец - v0, 0)
    n1 = max(round(ост*0.45/v1), 1); n2 = max(round((ост-n1*v1)/v2), 1)
    спец = v0 + v1*n1 + v2*n2
...
aq_rep = re.search(r'AQ\s*:\s*([\d.]+)', rep)
aq_form = re.search(r'id="acq_rate"[^>]*value="([\d.]+)"', calc)
проверка('ставка эквайринга одинакова в анкете и отчёте',
         bool(aq_rep and aq_form) and abs(float(aq_rep.group(1)) * 100 - float(aq_form.group(1))) < 1e-9,
         f'отчёт {float(aq_rep.group(1))*100:.1f} % · анкета {aq_form.group(1)} %')
...
def детей(s):
    ...
    return дети
n = детей(rep)
проверка('в .wp ровно 24 блока (как ждёт собрать.py)', n == 24, f'{n}')
...
for имя, шаб in (('DEV_NO_BLUR', r'DEV_NO_BLUR'), ('YM_ID', r'YM_ID'), ('PAY_URL', r'PAY_URL')):
    if not re.search(шаб, calc): замечание(f'{имя} отсутствует в calc.html')
if "THANKS_URL='#thanks'" in rep: замечание('THANKS_URL в отчёте — заглушка #thanks')
