...
Текущая разметка (пересобрана пользователем 21.08.2026):
  № · Наименование · ID · Назначение · Тип · Определение ·
  Значение/формула · Описание алгоритма · Формула (пример расчёта)
...
"""
import re, html, datetime, shutil, zipfile, os
import openpyxl

# Корень определяется от расположения этого файла. Раньше был зашит
# /home/user/schetix, поэтому после переезда репозитория обычный запуск падал.
КОРЕНЬ = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
КНИГА = os.path.join(КОРЕНЬ, 'Калькулятор_ставки_часа.xlsx')
ЛИСТ = '01_Глоссарий'

ШАПКА = ['№', 'Наименование', 'ID', 'Назначение', 'Тип', 'Определение',
         'Значение/формула', 'Описание алгоритма', 'Формула (пример расчёта)']
ПОЛЯ = ['имя', 'ид', 'назначение', 'тип', 'понятие', 'знач', 'алго', 'пример']

ЗАГОЛОВКИ = {
    '№': 'номер', 'наименование': 'имя', 'id': 'ид', 'назначение': 'назначение',
    'тип': 'тип', 'понятие': 'понятие', 'определение': 'понятие',
    'значение/формула': 'знач', 'значение / формула': 'знач',
    'описание алгоритма': 'алго', 'формула (пример расчёта)': 'пример',
}
...
# ══ ЧТЕНИЕ ═══════════════════════════════════════════════════════════
def читать():
    """Читает лист по заголовкам столбцов и сохраняет абзацы.

    Строка ``РАЗДЕЛ · …`` открывает крупный раздел, строка
    ``ПОДРАЗДЕЛ · …`` — подраздел. Заголовок столбцов может повторяться
    перед каждым абзацем, как в прежнем глоссарии.
    """
    ...
    for r in ws.iter_rows(values_only=True):
        ...
        первая = зн[0] if зн else ''
        if первая.startswith('РАЗДЕЛ ·'):
            раздел = первая.split('·', 1)[1].strip()
            подраздел = ''
            continue
        if первая.startswith('ПОДРАЗДЕЛ ·'):
            подраздел = первая.split('·', 1)[1].strip()
            continue
        ...
        с = {п: '' for п in ПОЛЯ}
        с['раздел'], с['подраздел'] = раздел, подраздел
...
def писать(строки):
    есть_абзацы = any(с.get('раздел') or с.get('подраздел') for с in строки)
    if not есть_абзацы:
        из = [ШАПКА]
        for с in строки:
            из.append([str(с['№']) if с.get('№') else ''] + [с.get(п, '') for п in ПОЛЯ])
    else:
        из = [['01 · ГЛОССАРИЙ — единый реестр понятий, параметров и обозначений'],
              ['Основной источник данных калькулятора. Разделы и подразделы '
               'отделены абзацами; HTML собирается из этого листа.'], []]
        прежний_раздел = прежний_подраздел = None
        for с in строки:
            раздел = с.get('раздел', '')
            подраздел = с.get('подраздел', '')
            if раздел != прежний_раздел:
                if прежний_раздел is not None: из.append([])
                из.append(['РАЗДЕЛ · ' + (раздел or 'Без раздела')])
                прежний_раздел, прежний_подраздел = раздел, None
            if подраздел != прежний_подраздел:
                из.append(['ПОДРАЗДЕЛ · ' + (подраздел or 'Без подраздела')])
                из.append(ШАПКА)
                прежний_подраздел = подраздел
    ...
    xml = ('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n'
           ...
           '<col min="9" max="9" width="42" customWidth="1"/></cols>'
...
ЦВЕТ_ТИПА = {
    'meta': ('#EFE8FB', '#5B34A8'),
    'служебное': ('#F1F3F6', '#5B6673'),
    'ui': ('#E8F5EC', '#1B7F3B'),
    'marker': ('#EFE8FB', '#5B34A8'),
    'data_type': ('#E7EFFB', '#1B4F9C'),
    'const': ('#EEF1FA', '#2C3E82'),
    'default': ('#FFF3D6', '#92400E'),
    'calc': ('#E7F0FA', '#2A6FAB'),
    'switch': ('#FCE7F3', '#9D174D'),
    'понятие': ('#F5FBF7', '#14532D'),
}
...
td.пример{width:190px;color:#5B6673;font-size:13px}
.мет{display:inline-block;padding:2px 9px;border-radius:20px;font-size:12px;font-weight:600}
.нет td{background:#FFFBFC}
.секция td{background:#2C3648;color:#fff;font-size:17px;font-weight:700;padding:14px 12px;
letter-spacing:-.01em;border-bottom:0}
.подсекция td{background:#E9EDF3;color:#344054;font-size:13px;font-weight:700;padding:9px 12px;
text-transform:none;border-bottom:1px solid #D5DAE1}
.повтор-шапки th{background:#667085}
...
.скрыт{display:none}
    ...
    o.append('<p class="под">Источник — лист <code>01_Глоссарий</code> книги. '
             'Тёмные строки отделяют крупные разделы, светлые — подразделы. '
             'Столбец «Тип» хранит тип записи прежнего глоссария: meta, '
             'служебное, ui, const, default, calc, switch или понятие.</p>')
    ...
    есть_абзацы = any(с.get('раздел') or с.get('подраздел') for с in строки)
    if not есть_абзацы:
        o.append('<tr class="повтор-шапки">' + ''.join(f'<th>{з}</th>' for з in ШАПКА) + '</tr>')
    прежний_раздел = прежний_подраздел = None
    for с in строки:
        раздел = с.get('раздел', '')
        подраздел = с.get('подраздел', '')
        if есть_абзацы and раздел != прежний_раздел:
            o.append(f'<tr class="секция"><td colspan="9">{э(раздел or "Без раздела")}</td></tr>')
            прежний_раздел, прежний_подраздел = раздел, None
        if есть_абзацы and подраздел != прежний_подраздел:
            o.append(f'<tr class="подсекция"><td colspan="9">{э(подраздел or "Без подраздела")}</td></tr>')
            o.append('<tr class="повтор-шапки">' + ''.join(f'<th>{з}</th>' for з in ШАПКА) + '</tr>')
            прежний_подраздел = подраздел
        ...
        o.append(f'<tr{кл} data-т="{э(с["тип"])}" data-п="{"нет" if not с["понятие"] else "да"}">'
                 ...
                 f'<td class="алго">{э(с["алго"])}</td>'
                 f'<td class="пример">{э(с["пример"])}</td></tr>')
    o.append('</table>')
...
if __name__ == '__main__':
    ...
    if '--в-таблицу' in sys.argv:
        писать(строки)
    путь = os.path.join(КОРЕНЬ, 'модель', 'глоссарий.html')
